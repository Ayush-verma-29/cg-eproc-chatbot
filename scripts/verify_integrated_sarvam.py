# scripts/verify_integrated_sarvam.py
import sys
...
def verify():
    # Force settings translation model to Sarvam-1 for this integration test
    ...
    print(f"Testing integration with translation model: {settings.TRANSLATION_MODEL}")
    print("Calling translate_to_hindi...")
    
    translated = language_service.translate_to_hindi(english_text)
    
    print("\n--- TRANSLATION RESULTS ---")
    print(translated)
    print("----------------------------")
    
    # Save the output to verify_sarvam_output.txt
    with open("scripts/verify_sarvam_output.txt", "w", encoding="utf-8") as f:
        f.write(translated)
    print("\nSaved output to scripts/verify_sarvam_output.txt")
